...
function initERD(erdDiv, width, height) {
    paper = new Paper({
        el: erdDiv,
        width: width,
        //height: height,
...
function createEntitytype(ent) {
    /* create entitytype */
    if (ent.options == undefined || ent.options == null) {
        ent.options = []
    }
    if (ent.attributes == undefined || ent.attributes == null) {
        ent.attributes = {}
    }
...
function createRelationship(rel) {
    if (rel.options == undefined || rel.options == null) {
        rel.options = []
    }
    ...
    for (a in rel.attributes) {
        ...
        if (att.options == undefined || att.options == null) {
            att.options = []
        }
...
graph.on('change:position', function (cell) {
    if (cell.attributes.position == undefined) {
        return
    }
    ...
    if (cell.attributes.position.x < 2) {
        cell.attributes.position.x = 2;
    }
...
//createEntitytype({"_e":"Wettbewerbe", attributes:[{_a:"Bezeichnung",options:["primary"]},{_a:"Datum",options:["primary"]}]});
//createRelationship({_e:["Monster", "Wettbewerbe"], card:["N","M"], _r:"nehmen teil"})
